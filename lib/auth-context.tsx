...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        if (savedUser) {
          try {
            ...
            if (savedProfile) {
              ...
              fetchUserProfile(parsedUser.id, parsedUser);
            }
  ...
  const fetchUserProfile = async (userId: string, userData?: any) => {
    try {
      ...
      setUserProfile(data);
      if (typeof window !== "undefined") {
        localStorage.setItem("auth:profile", JSON.stringify(data));
      }
    } catch (error) {
      console.error("Error fetching user profile:", error);
    }
  ...
  const loginWithName = async (
    ...
    try {
      ...
      if (!userData) {
        console.error(
          "Login error: Usuario no encontrado o credenciales incorrectas"
        );
        setIsLoading(false);
        return false;
      }
      ...
      if (typeof window !== "undefined") {
        ...
        localStorage.setItem("auth:profile", JSON.stringify(userData));
      }
      setIsLoading(false);
      return true;
    } catch (error) {
