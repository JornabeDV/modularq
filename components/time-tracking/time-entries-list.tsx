...
export function TimeEntriesList({
  ...
  const calculateHoursFromTimes = useCallback(
  ...
  const renderedEntries = useMemo(() => {
    return entries.map((entry) => {
      ...
      if (!hoursToDisplay || hoursToDisplay === 0 || isNaN(hoursToDisplay)) {
        hoursToDisplay = calculateHoursFromTimes(
          entry.start_time,
          entry.end_time
        );
      }
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                {entry.project && (
                  <div className="text-xs">{entry.project.name}</div>
                )}
              </div>
              {entry.description && (
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground">
          {taskId
            ? "No se han registrado horas para esta tarea"
            : "No se han registrado horas de trabajo"}
        </p>
      </div>
    );
  }
