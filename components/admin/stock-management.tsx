...
export function StockManagement() {
  ...
  const filteredMaterials =
    materials?.filter((material) => {
      ...
      const matchesLowStock =
        !lowStockOnly || material.stockQuantity <= material.minStock;
  ...
  const lowStockMaterials =
    materials?.filter((m) => m.stockQuantity <= m.minStock) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <MaterialForm
        ...
        onSubmit={(data) =>
          editingMaterial && handleUpdateMaterial(editingMaterial.id, data)
        }
        isEditing={true}
