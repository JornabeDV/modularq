...
export function RentalModuleDetail({ moduleId }: { moduleId: string }) {
  ...
  const statusInfo = STATUS_LABELS[module.status] || { label: module.status, color: "" }
  const activeContract = module.current_contract || module.contracts?.find((c: any) => c.status === "active")

  const handleReturn = async () => {
    if (!activeContract) return
    setReturning(true)
    try {
      const res = await fetch(`/api/rental-contracts/${activeContract.id}/return`, {
        method: "POST",
      ...
      toast({ title: "Devolución registrada", description: "El módulo volvió a estar disponible" })
      setShowReturnDialog(false)
      refetch()
      fetchContracts({ rental_module_id: moduleId })
    } catch (err) {
      console.error(err)
      toast({ title: "Error", description: "No se pudo procesar la devolución", variant: "destructive" })
    } finally {
      setReturning(false)
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 sm:ml-auto">
          ...
          {(module.status === "available" || module.status === "maintenance") && (
            <Button size="sm" onClick={() => setShowContractForm(true)} className="cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-sm">{LOCATION_LABELS[module.location] || module.location || "—"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Proyecto Origen</span>
              {module.project?.id ? (
                <Link href={`/admin/projects/${module.project.id}`} className="text-sm text-primary hover:underline">
                  {module.project.name}
                </Link>
              ) : (
            ...
            <div className="flex justify-between">
              ...
              <span className="text-sm">{module.height}m × {module.width}m × {module.depth}m</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Modulación</span>
              <span className="text-sm">{module.modulation}</span>
            </div>
            {module.notes && (
        ...
        {activeContract ? (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-medium">Contrato Activo</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="text-xs text-muted-foreground">Cliente</p>
                <p className="text-base font-semibold">{activeContract.client?.company_name || "—"}</p>
              </div>
              {(activeContract.client?.phone || activeContract.client?.email) && (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm">{formatProjectDate(activeContract.start_date)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Finalización</p>
                  <p className="text-sm">{formatProjectDate(activeContract.end_date) || "Sin fecha"}</p>
                </div>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Precio Mensual</p>
                <p className="text-sm font-medium">${activeContract.monthly_price} {activeContract.currency}</p>
              </div>
              {activeContract.delivery_notes && (
                <div className="pt-2 border-t">
                  <p className="text-xs text-muted-foreground">Notas de entrega</p>
                  <p className="text-sm mt-1">{activeContract.delivery_notes}</p>
                </div>
              )}
              <Button
                className="w-full"
