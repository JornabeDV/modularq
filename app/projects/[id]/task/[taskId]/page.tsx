...
export default function TaskDetailPage() {
  ...
  const { projectTasks, updateProjectTask, loading } =
  ...
  // Sincronizar progreso al 100% cuando la tarea está completada
  useEffect(() => {
    if (task && task.status === "completed" && progressPercentage !== 100) {
      setProgressPercentage(100);
      updateProjectTask(task.id, { progressPercentage: 100 }, true, user?.id);
    }
